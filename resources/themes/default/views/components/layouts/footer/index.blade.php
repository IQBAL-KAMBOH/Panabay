{!! view_render_event('bagisto.shop.layout.footer.before') !!}

<!--
    The category repository is injected directly here because there is no way
    to retrieve it from the view composer, as this is an anonymous component.
-->
@inject('themeCustomizationRepository', 'Webkul\Theme\Repositories\ThemeCustomizationRepository')

<!--
    This code needs to be refactored to reduce the amount of PHP in the Blade
    template as much as possible.
-->
@php
$channel = core()->getCurrentChannel();

$customization = $themeCustomizationRepository->findOneWhere([
'type' => 'footer_links',
'status' => 1,
'theme_code' => $channel->theme,
'channel_id' => $channel->id,
]);
@endphp

<footer class="mt-9 bg-navyBlue text-white max-sm:mt-10 p-4">
    <div class="justify-between gap-x-6 gap-y-8 p-[60px] max-1060:flex-col-reverse max-md:gap-5 max-md:p-8 max-sm:px-4 max-sm:py-5">

        @if(false)
        <!-- News Letter subscription -->
        @if (core()->getConfigData('customer.settings.newsletter.subscription'))
        <div class="flex   gap-2.5 justify-center">
            <div>
                <x-shop::form
                    :action="route('shop.subscription.store')"
                    class="mt-2.5 rounded max-sm:mt-0">
                    <div class="relative w-full">
                        <x-shop::form.control-group.control
                            type="email"
                            class="block mt-2 mb-2 w-[420px] max-w-full rounded-xl border-2 border-[#e9decc] bg-[#F1EADF] px-5 py-4 text-base max-1060:w-full max-md:p-3.5 max-sm:mb-0 max-sm:rounded-lg max-sm:border-2 max-sm:p-2 max-sm:text-sm"
                            name="email"
                            rules="required|email"
                            label="Email"
                            :aria-label="trans('shop::app.components.layouts.footer.email')"
                            placeholder="email@example.com" />

                        <x-shop::form.control-group.error control-name="email" />

                        <button
                            type="submit"
                            class="block w-[420px] max-w-full rounded-xl border-2 bg-white px-5 py-4 text-navyBlue max-1060:w-full max-md:p-3.5 max-sm:mb-0 max-sm:rounded-lg max-sm:border-2 max-sm:p-2 max-sm:text-sm  mt-2">
                            @lang('shop::app.components.layouts.footer.subscribe')
                        </button>
                    </div>
                </x-shop::form>
            </div>
        </div>
        @endif
        @endif
        @php
        $footerLinks = [
        ['title' => 'All Departments', 'url' => '#'],
        ['title' => 'Store Directory', 'url' => '#'],
        ['title' => 'Careers', 'url' => '#'],
        ['title' => 'Our Company', 'url' => '#'],
        ['title' => 'Sell on Walmart.com', 'url' => '#'],
        ['title' => 'Help', 'url' => '#'],
        ['title' => 'Product Recalls', 'url' => '#'],
        ['title' => 'Accessibility', 'url' => '#'],
        ['title' => 'Tax Exempt Program', 'url' => '#'],
        ['title' => 'Get the Walmart App', 'url' => '#'],
        ['title' => 'Safety Data Sheet', 'url' => '#'],
        ['title' => 'Terms of Use', 'url' => '#'],
        ['title' => 'Privacy Notice', 'url' => '#'],
        ['title' => 'California Supply Chain Act', 'url' => '#'],
        [
        'title' => '<svg class="h-4 w-7 inline-block -mt-1 mr-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 28 16" fill="currentColor" role="img" aria-hidden="true">
            <path d="M14 0C6.268 0 0 2.238 0 5c0 1.954 3.03 3.65 7.237 4.545a.75.75 0 01.526 1.378C6.185 11.97 3.999 13.682 4 15.53a.5.5 0 00.5.47h19a.5.5 0 00.5-.47c.001-1.848-2.184-3.56-3.763-4.607a.75.75 0 01.526-1.378C24.969 8.65 28 6.954 28 5c0-2.762-6.268-5-14-5z" fill-rule="evenodd" clip-rule="evenodd"></path>
            <path d="M18.528 10.288l-7.07-7.07a.985.985 0 10-1.393 1.393l7.07 7.07a.985.985 0 001.393-1.393z" fill="#fff"></path>
            <path d="M10.065 10.288l7.07-7.07a.985.985 0 111.393 1.393l-7.07 7.07a.985.985 0 01-1.393-1.393z" fill="#fff"></path>
        </svg>Your Privacy Choices',
        'url' => '#'
        ],
        ['title' => 'Notice at Collection', 'url' => '#'],
        ['title' => 'AdChoices', 'url' => '#'],
        ['title' => 'Consumer Health Data Privacy Notices', 'url' => '#'],
        ['title' => 'Brand Shop Directory', 'url' => '#'],
        ['title' => 'Pharmacy', 'url' => '#'],
        ['title' => 'Walmart Business', 'url' => '#'],
        ['title' => '#IYWYK', 'url' => '#'],
        ['title' => 'Delete Account', 'url' => '#'],
        ];
        @endphp
        <!-- For Desktop View -->
        <div class="flex py-4 flex-wrap text-center items-center gap-24 max-1180:gap-6 max-1060:hidden">
            @if ($customization?->options)

            <ul class="flex flex-wrap justify-center items-start gap-x-4 gap-y-6 sm:gap-x-6">
                @foreach ($customization->options as $footerLinkSection)

                @php
                usort($footerLinkSection, function ($a, $b) {
                return $a['sort_order'] - $b['sort_order'];
                });
                @endphp

                @if(false)
                @foreach ($footerLinkSection as $link)
                <li>
                    <a href="{{ $link['url'] }}">
                        {{ $link['title'] }}
                    </a>
                </li>
                @endforeach
                @endif

                @endforeach
                @foreach ($footerLinks as $link)
                <li>
                    <a
                        href="{{ $link['url'] }}"
                        class="text-sm font-medium max-sm:text-xs">
                        {{-- Use {!! !!} to render the SVG icon correctly --}}
                        {!! $link['title'] !!}
                    </a>
                </li>
                @endforeach
            </ul>
            @endif
        </div>

        <!-- For Mobile view -->
        <x-shop::accordion
            :is-active="false"
            class="hidden mt-4 mb-4 !w-full rounded-xl !border-2 !border-[#e9decc] max-1060:block max-sm:rounded-lg">
            <x-slot:header class="rounded-t-lg bg-[#F1EADF] font-medium max-md:p-2.5 max-sm:px-3 max-sm:py-2 max-sm:text-sm">
                @lang('shop::app.components.layouts.footer.footer-content')
                </x-slot>

                <x-slot:content class="flex justify-between !bg-transparent !p-4">
                     <ul class="grid gap-5 text-sm">
                     @foreach ($footerLinks as $link)
                        <li>
                            <a
                                href="{{ $link['url'] }}"
                                class="text-sm font-medium max-sm:text-xs">
                                {{-- Use {!! !!} to render the SVG icon correctly --}}
                                {!! $link['title'] !!}
                            </a>
                        </li>
                        @endforeach
                    </ul>
                    @if ($customization?->options)
                    @foreach ($customization->options as $footerLinkSection)
                    <ul class="grid gap-5 text-sm">
                        @php
                        usort($footerLinkSection, function ($a, $b) {
                        return $a['sort_order'] - $b['sort_order'];
                        });
                        @endphp

                        @if(false)
                        @foreach ($footerLinkSection as $link)
                        <li>
                            <a
                                href="{{ $link['url'] }}"
                                class="text-sm font-medium max-sm:text-xs">
                                {{ $link['title'] }}
                            </a>
                        </li>
                        @endforeach
                        @endif

                    </ul>
                    @endforeach
                    @endif
                    </x-slot>
        </x-shop::accordion>






    </div>

    <div class="">
        {!! view_render_event('bagisto.shop.layout.footer.footer_text.before') !!}

        <p class="text-sm text-white text-center">
            @lang('shop::app.components.layouts.footer.footer-text', ['current_year'=> date('Y') ])
        </p>

        {!! view_render_event('bagisto.shop.layout.footer.footer_text.after') !!}
    </div>
</footer>

{!! view_render_event('bagisto.shop.layout.footer.after') !!}